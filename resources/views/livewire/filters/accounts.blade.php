<section class="container" x-data="people">
    <div class="grid  grid-cols-2 gap-5 py-10">
        <section id="peopleView" class="col-span-1 justify-center px-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                ...
                <div @click="showConversation({{$person->id}})"
                    class="p-5 col-span-1  rounded-lg  text-[0.8125rem] leading-5 text-slate-900 shadow-xl shadow-black/5 ring-1 ring-slate-700/10"
                   :class="isSelected(@js($person->id)) ? 'bg-blue-400 text-white':'bg-white'">
                    <span>Profile: {{$person->name}}</span>
                    <div class="flex border-t py-2 gap-x-4">
                        <div><a href="mailto:{{$person->email}}">
                                <i class="fa-solid fa-envelope"></i> Email
                            </a>
                        </div>
                        <div><a href="tel:{{$person->phone}}">
        ...
        <section id="chatHistory" class="col-span-1">
         ...
         <div id="messages" class="flex flex-col space-y-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch">
            ...
            @foreach($trail as $chat)
            <div class="chat-message">
               <div class="flex items-end justify-end">
                  <div class="flex flex-col space-y-2 text-xs max-w-xs mx-2 order-1 items-end">
                     <div><span class="px-4 py-2 rounded-lg inline-block rounded-br-none bg-blue-600 text-white ">{{$chat->query}}</span></div>
                  </div>
               </div>
            </div>
            <div class="chat-message">
               <div class="flex items-end">
                  ...
                  <i class="fa-brands fa-2x fa-bots"></i>
                     <div><span class="px-4 py-2 rounded-lg inline-block rounded-bl-none bg-gray-300 text-gray-600">{{$chat->answer}}</span></div>
                  </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('people', () => ({
            selectedProfile:@entangle('selectedProfile'),
            isSelected(personId){
                return this.selectedProfile==personId;
            },
            showConversation(personId){
                this.selectedProfile=personId;
            }
        }))
    });
</script>
